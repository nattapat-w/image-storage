...
func (r *TagRepo) SetImageTags(userID, imageID string, tagNames []string, now string) error {
	...
	if err := r.store.DB.QueryRow(
		`SELECT user_id FROM images WHERE id = $1 AND deleted_at IS NULL`,
		imageID,
	...
	for _, name := range tagNames {
		...
		if _, err := tx.Exec(`INSERT INTO image_tags (image_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, imageID, tagID); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func upsertTag(tx *sql.Tx, userID, name, now string) (string, error) {
	var id string
	err := tx.QueryRow(`SELECT id FROM tags WHERE user_id = $1 AND name = $2`, userID, name).Scan(&id)
	if err == nil {
		return id, nil
	}
	...
	id = uuid.NewString()
	_, err = tx.Exec(`INSERT INTO tags (id, user_id, name, created_at) VALUES ($1, $2, $3, $4)`, id, userID, name, now)
	return id, err
}
