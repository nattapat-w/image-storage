...
func (r *FolderRepo) ListChildren(userID string, parentID *string) ([]domain.Folder, error) {
	...
	if parentID == nil {
		rows, err = r.store.DB.Query(
			`SELECT id, parent_id, name, created_at, updated_at FROM folders WHERE user_id = $1 AND parent_id IS NULL ORDER BY name`,
			userID,
		)
	} else {
		rows, err = r.store.DB.Query(
			`SELECT id, parent_id, name, created_at, updated_at FROM folders WHERE user_id = $1 AND parent_id = $2 ORDER BY name`,
			userID, *parentID,
		)
	}
	if err != nil {
...
func (r *FolderRepo) Get(userID, id string) (domain.Folder, error) {
	row := r.store.DB.QueryRow(
		`SELECT id, parent_id, name, created_at, updated_at FROM folders WHERE id = $1 AND user_id = $2`,
		id, userID,
	)
