...
func (r *ShareFolderRepo) SetSharingEnabled(ownerID, folderID string, enabled bool, updatedAt string) error {
	res, err := r.store.DB.Exec(
		`UPDATE folders SET is_share_folder = $1, updated_at = $2 WHERE id = $3 AND user_id = $4`,
		enabled, updatedAt, folderID, ownerID,
	)
	if err != nil {
		return err
	}
...
func (r *ShareFolderRepo) IsSharingRoot(folderID string) (bool, error) {
	var isShare bool
	err := r.store.DB.QueryRow(`SELECT is_share_folder FROM folders WHERE id = $1`, folderID).Scan(&isShare)
...
func (r *ShareFolderRepo) GetFolderOwner(folderID string) (string, error) {
	var owner string
	err := r.store.DB.QueryRow(`SELECT user_id FROM folders WHERE id = $1`, folderID).Scan(&owner)
	if err == sql.ErrNoRows {
		return "", domain.ErrNotFound
	}
	return owner, err
...
func (r *ShareFolderRepo) IsFolderInTree(folderID, rootID string) (bool, error) {
	...
	for i := 0; i < 256; i++ {
		...
		err := r.store.DB.QueryRow(`SELECT parent_id FROM folders WHERE id = $1`, cur).Scan(&parent)
		if err == sql.ErrNoRows {
			return false, nil
		}
		if err != nil {
...
func (r *ShareFolderRepo) ResolveShareRoot(folderID string) (rootID, ownerID string, ok bool, err error) {
	...
	for i := 0; i < 256; i++ {
		...
		err := r.store.DB.QueryRow(
			`SELECT parent_id, is_share_folder, user_id FROM folders WHERE id = $1`, cur,
...
func (r *ShareFolderRepo) AddMember(id, folderID, userID, role, invitedBy, joinedAt string) error {
	_, err := r.store.DB.Exec(
		`INSERT INTO folder_members (id, folder_id, user_id, role, invited_by, joined_at) VALUES ($1, $2, $3, $4, $5, $6)
ON CONFLICT (folder_id, user_id) DO NOTHING`,
		id, folderID, userID, role, nullIfEmpty(invitedBy), joinedAt,
	)
	return err
}
...
func (r *ShareFolderRepo) RemoveMember(folderID, userID string) error {
	res, err := r.store.DB.Exec(`DELETE FROM folder_members WHERE folder_id = $1 AND user_id = $2`, folderID, userID)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
...
func (r *ShareFolderRepo) IsMember(folderID, userID string) (bool, error) {
	var n int
	err := r.store.DB.QueryRow(
		`SELECT 1 FROM folder_members WHERE folder_id = $1 AND user_id = $2`, folderID, userID,
	).Scan(&n)
	if err == sql.ErrNoRows {
		return false, nil
	}
...
WHERE fm.user_id = $1 AND f.is_share_folder = true
ORDER BY f.name`, userID)
	...
	for rows.Next() {
		...
		var parent sql.NullString
		if err := rows.Scan(
			&e.Folder.ID, &parent, &e.Folder.Name, &e.Folder.IsShareFolder,
...
func (r *ShareFolderRepo) CreateInvite(inv domain.FolderInvite) error {
	_, err := r.store.DB.Exec(
		`INSERT INTO folder_invites (id, folder_id, email, token, invited_by, expires_at, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
...
SELECT id, folder_id, email, token, invited_by, expires_at, accepted_at, created_at
FROM folder_invites
WHERE folder_id = $1 AND accepted_at IS NULL
...
SELECT id, folder_id, email, token, invited_by, expires_at, accepted_at, created_at
FROM folder_invites WHERE token = $1`, token)
	return scanInviteRow(row)
}

func (r *ShareFolderRepo) MarkInviteAccepted(inviteID, acceptedAt string) error {
	res, err := r.store.DB.Exec(`UPDATE folder_invites SET accepted_at = $1 WHERE id = $2 AND accepted_at IS NULL`, acceptedAt, inviteID)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
...
WHERE id = $1 AND folder_id IN (SELECT id FROM folders WHERE user_id = $2)`, inviteID, ownerID)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
...
func (r *ShareFolderRepo) ClearSharingData(folderID string) error {
	if _, err := r.store.DB.Exec(`DELETE FROM folder_members WHERE folder_id = $1`, folderID); err != nil {
		return err
	}
	_, err := r.store.DB.Exec(`DELETE FROM folder_invites WHERE folder_id = $1`, folderID)
	return err
}
...
func (r *ShareFolderRepo) assertSharingRootOwned(ownerID, folderID string) error {
	var uid string
	var isShare bool
	err := r.store.DB.QueryRow(`SELECT user_id, is_share_folder FROM folders WHERE id = $1`, folderID).Scan(&uid, &isShare)
	if err == sql.ErrNoRows {
		return domain.ErrNotFound
	}
	if err != nil {
