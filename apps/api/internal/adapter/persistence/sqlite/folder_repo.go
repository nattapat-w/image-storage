...
func (r *FolderRepo) Delete(userID, id string) error {
	...
	_, _ = r.store.DB.Exec(
		`DELETE FROM shares WHERE user_id = ? AND resource_type = ? AND resource_id = ?`,
		userID, domain.ResourceFolder, id,
	)
	res, err := r.store.DB.Exec(`DELETE FROM folders WHERE id = ? AND user_id = ?`, id, userID)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
...
func (r *FolderRepo) PurgeAll(userID string) (int, error) {
	_, err := r.store.DB.Exec(
		`DELETE FROM shares WHERE user_id = ? AND resource_type = ?`,
		userID, domain.ResourceFolder,
	)
	if err != nil {
		return 0, err
	}
	res, err := r.store.DB.Exec(`DELETE FROM folders WHERE user_id = ?`, userID)
	if err != nil {
		return 0, err
	}
	n, _ := res.RowsAffected()
...
func (r *FolderRepo) StatsByUser(userID string) (map[string]domain.FolderStat, error) {
	rows, err := r.store.DB.Query(`SELECT id, parent_id FROM folders WHERE user_id = ?`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	imgRows, err := r.store.DB.Query(
		`SELECT folder_id, COUNT(*), COALESCE(SUM(size), 0) FROM images WHERE user_id = ? AND deleted_at IS NULL AND folder_id IS NOT NULL GROUP BY folder_id`,
		userID,
	)
	if err != nil {
		return nil, err
